...
# Matrice Poisson (0-9 goluri pentru acuratețe sporită)
max_goals = 10
goals = np.arange(max_goals)
pmf_home = poisson.pmf(goals, exp_g_home)
pmf_away = poisson.pmf(goals, exp_g_away)
poisson_matrix = np.outer(pmf_home, pmf_away)

# Calcul Probabilități 1X2
...
# Calcul Over 2.5 & GG (Simplificat)
rows, cols = np.indices((max_goals, max_goals))
prob_o25 = float(np.sum(poisson_matrix[rows + cols > 2]) * 100)
prob_gg = float(np.sum(poisson_matrix[1:, 1:]) * 100)
